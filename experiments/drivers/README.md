...
- `snowmelt`: `swe_7day` is falling over the window (last non-NaN value below the first) and `flow_7day_avg` on the peak date is at or above the full-frame median (496 cfs).
- `rain`: `precip_7day` on any day in the window is at or above the full-frame 90th percentile (`RAIN_QUANTILE`, 0.173 in/day) and flow on the peak date is below the median.
...
## Episodes, run 2026-08-28

| Start | End | Days | Peak (mg/L) | Peak date | Driver | Forest | CatBoost |
|---|---|---|---|---|---|---|---|
| 2023-05-14 | 2023-07-17 | 65 | 7.3 | 2023-05-19 | snowmelt | caught, +2 days | caught, +5 days |
| 2024-04-15 | 2024-06-07 | 54 | 6.4 | 2024-05-18 | snowmelt | caught, +13 days | caught, +4 days |
...
- **Every melt episode is caught, but late.** On the two big melt years the forest first crossed 3 mg/L two days into the 2023 episode and thirteen days into 2024's; CatBoost five and four days in. On a 54-day episode that still leaves weeks of warning, but it is not the "four days ahead" of the deck; the lead is built into the features (USGS lag 2, SNOTEL lag 4) and the model spends it. The two short 2025 episodes were called one and four days early.
...
- **Episode recall and day recall answer different questions.** `rolling/` reports 2023 day-level recall of 0.23 (forest) and 0.11 (CatBoost) because most of the 65 days were predicted below 3; the episode was still "caught" because at least one day crossed. The operator's sentence is the episode one ("the model will tell you the flush has started"); the day one says how much of the flush it tracks.
